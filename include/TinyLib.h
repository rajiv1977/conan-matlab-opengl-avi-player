...
- implement orthonormal basis from direction vector:
http://orbit.dtu.dk/fedora/objects/orbit:113874/datastreams/file_75b66578-222e-4c7d-abdf-f7e255100209/content By Jeppe
Revall Frisvad, http://www.imm.dtu.dk/~jerf/,
...
rx_create_shader_from_file(GL_VERTEX_SHADER, path);                       - create a shader from a file, give a full
path, returns -1 on error rx_create_program(vert, frag, link);                                      - create a program,
linkes when link == true rx_create_program_with_attribs(vert, frag, 2, attr);                      - create a program
with attribs + LINKS THE PROGRAM rx_print_shader_link_info(prog)                                           - print the
program link info rx_print_shader_compile_info(vert)                                        - print the shader compile
info rx_create_texture(filepath)                                               - loads a png and creates a texture (only
when png is enabled) rx_get_uniform_location(prog, name)                                       - returns the location of
the uniform or returns -1 and logs something in debug mode rx_uniform_1i(prog, name, value) - set a 1i value - make sure
to activate the shader before calling, e.g. glUseProgram(prog) rx_uniform_1f(prog, name, value) - set a 1f value - make
sure to activate the shader before calling, e.g. glUseProgram(prog) rx_uniform_mat4fv(prog, name, count, trans, ptr) -
set a mat4fv - make sure to activate the shader before calling, e.g. glUseProgram(prog)

rx_create_png_screenshot(filepath)                                        - create a png file from the current
framebuffer (must have PNG enabled) rx_create_jpg_screenshot(filepath, quality)                               - create a
jpg file from the current framebuffer (must have JPG enabled)

Shader                                                                    - represents a GL shader; only works with
files! Shader.load(type, filepath)                                               - load the source for the shader from
file Shader.compile()                                                          - compiles the shader

Program                                                                   - represents a GL program
Program.add(Shader)                                                       - add a already compiled shader to the program
Program.create(type, filepath, extra)                                     - create a Shader for the given filepath.
extra is extra source code that gets prepended. Program.link(nattribs, char** attribs, int nfrags, const char** frags)
- link + bind the attrib locations and/or fragment locations Program.recompile() - recompiles the shader and links the
program again

VertexP                                                                   - vertex type for position data
VertexPC                                                                  - vertex type for position and color (4) data
VertexPT                                                                  - vertex type for position and texture coord
data VertexPT3                                                                 - vertex type for position and texture
coord with 3 elements (a q for projective mapping) VertexPTN - vertex type for position, texture and normal VertexPN -
vertex type for position and normals
...
Painter                                                                   - simple helper to draw lines, circles,
rectangles, textures with GL 3.x Painter.init()                                                            - must be
called to ininitialize the GL-objects. Painter.clear()                                                           - clear
all added elements, resets the canvas Painter.draw()                                                            - draw
the added shapes Painter.rect(x, y, w, h)                                                  - draw a rectangle
...
Painter.width()                                                           - return the last set or queried viewport
width Painter.height()                                                          - return the last set of queried
viewport height
-
Painter.begin(GL_LINE_STRIP)                                              - begin drawing a type
Painter.vertex(x, y)                                                      - add a vertex to the "begin/end" section
Painter.end()                                                             - flush added vertices and make sure their
drawn
-
...
font.clear()                                                              - clear all previously added text (make sure
to clear() when you want to draw text that changes every time) font.write(10, 10, "Lorem ipsum_dolor sit") - write some
text at 10,10 font.draw()                                                               - draws all the batched writes
font.color(r, g, b, a)                                                    - set the font color
font.resize(w, h)                                                         - when the viewport size changes you must call
this.

IMAGES - define `ROXLU_USE_PNG` before including -                        - see
https://gist.github.com/roxlu/9b9d555cf784385d67ba for some loading examples
===================================================================================
...
bool rx_save_png("filename.png", pixels, 640, 480, 3, flip);                         - writes a png using lib png, set
flip = true if you want to flip horizontally int rx_load_png("filepath.png", &pix, width, height, nchannels) - load the
pixels, width, height and nchannels for the given filepath. make sure to delete pix (which is unsigned char*) int
rx_load_png("filepath.png", &pix, width, height, nchannels, &allocated, flags)   - load the pixels, the allocated param
should point to an integer that holds the number of bytes in the *pix buffer. It will try to reuse or reallocate this
buffer. Returns number of bytes in image buffer. You can pass RX_FLAG_LOAD_AS_RGBA if you want to force the output as
RGBA int rx_load_jpg("filepath.jpg", &pix, width, height, nchannels)                      - loads an jpg file, allocates
the buffer that you need to free, will return the number of bytes (int) int rx_load_jpg("filepath.jpg", &pix, width,
height, nchannels, &allocated)          - loads an jpg file. the allocated should point to an integer that holds the
number of bytes in the *pix buffer. It will try to reuse this or reallocate the buffer if needed. this will return the
number of allocated bytes bool rx_save_jpg("filepath.jpg", pixels, 640, 480, 3);                               - save a
jpg file
...
rx_string_id(std::string& v);                                            - generate an mostly unique uint32_t id for the
given string.
...
rx_strip_file_ext("/path/filename")                                      - removes the extension from the given
filename, including the dot. e.g. image.jpg becomes "image" rx_strip_dir("/path/filename") - removes the path from the
given path leaving only the filename rx_create_dir("/path/to")                                                - creates
the given directory rx_create_path("/a/b/c")                                                 - creates the path. all
subdirectories too) rx_norm_path("/some/path/")                                              - creates a normalized,
cross platform path. always pass in forward slashes; on windows you'll get backslashes rx_get_files("/path/", "jpg") -
returns a std::vector<std::string> with the files found in the given dir which have the given extension.
rx_get_files("/path/", "*")                                              - returns all the files in the path.
rx_get_file_ext("/file/path.jpg")                                        - returns the extension of a file ("jpg",
"gif", etc..)

rx_split("string", '/')                                                  - splits a string on the given character
returning std::vector<std::string> rx_string_replace("string", "replace", "with")                           - replaces
characters or string from one to another rx_to_lower("String With Caps")                                          -
returns the lowercased version of the given string.

rx_hrtime()                                                              - high resolution timer (time in nano sec)
rx_millis()                                                              - returns the elapsed millis since the first
call as float, 1000 millis returns 1.0 rx_strftime("%Y/%m%d/")                                                  -
strftime wrapper rx_get_time_string()                                                     - get a unique string with the
current date time: 2014.01.16_13.12_883  yyyy.mm.dd.hh.ss.iii (ii =  millis) rx_get_date_string() - get a unique string
with the current date 2014.01.16 rx_get_year()                                                            - get the
current year as int, e.g. 2014 rx_get_month()                                                           - get the
current month as int [00-11] rx_get_day()                                                             - get the day of
the month [00-31] rx_get_hour()                                                            - get the hour of day [00-23]
rx_get_minute()                                                          - get the minuts of the hours, [00-59]

rx_rgb_to_hsv(r,g,b,h,s,v)                                               - convert rgb in range 0-1 to hsv in the same
range. h,s,v are references rx_rgb_to_hsv(rgb, hsv)                                                  - convert given
vector, hsv will be set (reference) rx_rgb_to_hsv(rgb, float*)                                               - "",
different typed parameters rx_rgb_to_hsv(float*, float*)                                            - "", ""

rx_hsv_to_rgb(h,s,v,r,g,b)                                               - convert rgb to hsv all in 0-1 range. r,g,b
will be set, are references rx_hsv_to_rgb(hsv, rgb)                                                  - convert rgb to
hsv all in 0-1 range. rgb will be set, is a reference rx_hsv_to_rgb(hsv, float*) - "", "" rx_hsv_to_rgb(float,* float*)
- "", ""


LOG
===================================================================================
rx_log_init(std::string path = "")                                       - Initialize logging, creates a log file like:
log-[date-string].log in the same directory of the executable when path is not given. rx_log_disable_stdout() - Disable
output to stdout rx_log_enable_stdout()                                                   - Enable output to stdout
rx_log_set_level(int level)                                              - Set the log level, RX_LOG_LEVEL_{ALL, ERROR,
WARNING, VERBOSE} RX_VERBOSE("%s %s", "Hello", "world");                                   - Log a verbose message
...
bool rx_is_power_of_two(int n);                                          - returns true if the given number is a power
of two. float rx_map(val, inmin, inmax, outmin, outmax, clamp = true)            - map one range to another one and
clamp if necessary (true by default)
...
vec3
-----------------------------------------------------------------------------------
vec3 perpendicular(a)                                                    - get a perpendicular vector from the given
vec, this vector doesn't have to be normalized!, based on
http://lolengine.net/blog/2013/09/21/picking-orthogonal-vector-combing-coconuts bool intersect(a,b,c,d, &result) -
checks if two lines intersect line (b-a) and (d-c). resturns true when they intersect and it will set result to the
intersection point
...
mat4& mat4.perspective(fov, aspect, near, far)                           - create a RIGHT HANDED perspective projection
matrix mat4& mat4.lookat(eye, pos, up) void  mat4.print() float* mat4.ptr() - get a pointer to the data
...
AUDIO - define `ROXLU_USE_AUDIO`  (will need libcubeb: https://github.com/kinetiknz/cubeb and libsndfile, tested with
cubeb version 4bc13035b143ac299a58b8467b87212fc1f5958c )
===================================================================================
AudioPlayer player;                                                - creates a audio player
player.add(0, "bleep.wav");                                        - add the bleep.wav file (only 44100, 2 channel
support atm) player.add(1, "boing.wav");                                        - add the boing.wav with name '1'.
...
#define IS_INSIDE(mousex, mousey, x, y, w, h) \
...
#define RX_FLAG_LOAD_AS_RGBA                                                                                       \
    0x0001 /* can be used by image loading functions to convert loaded data directory to RGBA. See the rx_load_png \
              function. */
...
template <class T>
inline Vec2<T>::Vec2(const Vec2<T>& o)
    : x(o.x)
    , y(o.y)
{
}
template <class T>
inline Vec2<T>::Vec2(T f)
    : x(f)
    , y(f)
{
}
template <class T>
inline void Vec2<T>::set(T vx, T vy)
{
    x = vx;
    y = vy;
}
...
template <class T>
inline T& Vec2<T>::operator[](const unsigned int dx)
{
    return *(&x + dx);
}
template <class T>
...
template <class T>
inline Vec2<T> Vec2<T>::operator+(const Vec2<T>& o) const
{
...
template <class T>
inline Vec2<T> Vec2<T>::operator+(float s) const
{
...
template <class T>
inline Vec2<T> operator+(float s, const Vec2<T>& o)
{
    return Vec2<T>(s + o.x, s + o.y);
}
template <class T>
inline Vec2<T> operator-(float s, const Vec2<T>& o)
{
...
template <class T>
inline Vec2<T>& Vec2<T>::operator+=(const Vec2<T>& o)
{
    return *this = *this + o;
}
template <class T>
...
template <class T>
inline Vec2<T>& Vec2<T>::operator+=(float s)
{
    return *this = *this + s;
}
template <class T>
...
template <class T>
inline bool Vec2<T>::operator==(const Vec2<T>& o) const
{
    return x == o.x && y == o.y;
}
template <class T>
...
template <class T>
inline float length(const Vec2<T>& o)
{
...
template <class T>
inline float heighest(const Vec2<T>& v)
{
    return fmaxf(v.x, v.y);
}
template <class T>
inline float lowest(const Vec2<T>& v)
{
    return fminf(v.x, v.y);
}
template <class T>
inline Vec2<T> lowest(const Vec2<T>& a, const Vec2<T>& b)
{
    return Vec2<T>(fmaxf(a.x, b.x), fmaxf(a.y, b.y));
}
template <class T>
inline Vec2<T> heighest(const Vec2<T>& a, const Vec2<T>& b)
{
...
{
    return Vec2<T>(ceilf(v.x), ceilf(v.y));
}
template <class T>
inline Vec2<T> abs(const Vec2<T>& v)
{
    return Vec2<T>(fabsf(v.x), fabsf(v.y));
}
template <class T>
...
template <class T>
inline Vec3<T>::Vec3(const Vec3<T>& o)
    : x(o.x)
    , y(o.y)
    , z(o.z)
{
...
template <class T>
inline void Vec3<T>::set(const float xv, const float yv, const float zv)
{
    x = xv;
    y = yv;
    z = zv;
...
template <class T>
inline T& Vec3<T>::operator[](const unsigned int dx)
{
    return *(&x + dx);
}
template <class T>
...
template <class T>
inline Vec3<T> Vec3<T>::operator+(const Vec3<T>& o) const
{
...
template <class T>
inline Vec3<T> Vec3<T>::operator+(float s) const
{
...
template <class T>
inline Vec3<T> operator+(float s, const Vec3<T>& o)
{
    return Vec3<T>(s + o.x, s + o.y, s + o.z);
}
template <class T>
inline Vec3<T> operator-(float s, const Vec3<T>& o)
{
...
template <class T>
inline Vec3<T>& Vec3<T>::operator+=(const Vec3<T>& o)
{
    return *this = *this + o;
}
template <class T>
...
template <class T>
inline Vec3<T>& Vec3<T>::operator+=(float s)
{
    return *this = *this + s;
}
template <class T>
...
template <class T>
inline bool Vec3<T>::operator==(const Vec3<T>& o) const
{
...
template <class T>
inline float length(const Vec3<T>& o)
{
    return sqrtf(o.x * o.x + o.y * o.y + o.z * o.z);
}
template <class T>
...
template <class T>
inline float heighest(const Vec3<T>& v)
{
    return fmaxf(fmaxf(v.x, v.y), v.z);
}
template <class T>
inline float lowest(const Vec3<T>& v)
{
    return fminf(fminf(v.x, v.y), v.z);
}
template <class T>
inline Vec3<T> heighest(const Vec3<T>& a, const Vec3<T>& b)
{
    return Vec3<T>(fmaxf(a.x, b.x), fmaxf(a.y, b.y), fmaxf(a.z, b.z));
}
template <class T>
inline Vec3<T> lowest(const Vec3<T>& a, const Vec3<T>& b)
{
...
{
    return Vec3<T>(ceilf(v.x), ceilf(v.y), ceilf(v.z));
}
template <class T>
inline Vec3<T> abs(const Vec3<T>& v)
{
    return Vec3<T>(fabsf(v.x), fabsf(v.y), fabsf(v.z));
}
template <class T>
...
template <class T>
inline Vec3<T> perpendicular(const Vec3<T>& v)
{
    return abs(v.x) > abs(v.z) ? Vec3<T>(-v.y, v.x, 0.0) : Vec3<T>(0.0, -v.z, v.y);
}
template <class T>
inline void Vec3<T>::print()
{
    printf("x: %f, y: %f, z: %f\n", x, y, z);
}
...
template <class T>
inline Vec4<T>::Vec4(const Vec4<T>& o)
    : x(o.x)
    , y(o.y)
    , z(o.z)
    , w(o.w)
...
template <class T>
inline void Vec4<T>::set(const float xv, const float yv, const float zv, const float wv)
{
    x = xv;
    y = yv;
    z = zv;
...
template <class T>
inline T& Vec4<T>::operator[](const unsigned int dx)
{
    return *(&x + dx);
}
template <class T>
...
template <class T>
inline Vec4<T> Vec4<T>::operator+(const Vec4<T>& o) const
{
    return Vec4<T>(x + o.x, y + o.y, z + o.z, w + o.w);
}
template <class T>
inline Vec4<T> Vec4<T>::operator-(const Vec4<T>& o) const
{
...
template <class T>
inline Vec4<T> Vec4<T>::operator+(float s) const
{
...
template <class T>
inline Vec4<T> operator+(float s, const Vec4<T>& o)
{
    return Vec4<T>(s + o.x, s + o.y, s + o.z, s + o.w);
}
template <class T>
inline Vec4<T> operator-(float s, const Vec4<T>& o)
{
...
template <class T>
inline Vec4<T>& Vec4<T>::operator+=(const Vec4<T>& o)
{
    return *this = *this + o;
}
template <class T>
...
template <class T>
inline Vec4<T>& Vec4<T>::operator+=(float s)
{
    return *this = *this + s;
}
template <class T>
...
template <class T>
inline bool Vec4<T>::operator==(const Vec4<T>& o) const
{
...
template <class T>
inline float length(const Vec4<T>& o)
{
    return sqrtf(o.x * o.x + o.y * o.y + o.z * o.z + o.w * o.w);
}
template <class T>
...
template <class T>
inline float heighest(const Vec4<T>& v)
{
    return fmaxf(fmaxf(v.x, v.y), fmaxf(v.z, v.w));
}
template <class T>
inline float lowest(const Vec4<T>& v)
{
    return fminf(fminf(v.x, v.y), fminf(v.z, v.w));
}
template <class T>
inline Vec4<T> heighest(const Vec4<T>& a, const Vec4<T>& b)
{
    return Vec4<T>(fmaxf(a.x, b.x), fmaxf(a.y, b.y), fmaxf(a.z, b.z), fmaxf(a.w, b.w));
}
template <class T>
inline Vec4<T> lowest(const Vec4<T>& a, const Vec4<T>& b)
{
    return Vec4<T>(fminf(a.x, b.x), fminf(a.y, b.y), fminf(a.z, b.z), fminf(a.w, b.w));
}
template <class T>
inline Vec4<T> floor(const Vec4<T>& v)
{
    return Vec4<T>(floorf(v.x), floorf(v.y), floorf(v.z), floorf(v.w));
}
template <class T>
inline Vec4<T> ceil(const Vec4<T>& v)
{
    return Vec4<T>(ceilf(v.x), ceilf(v.y), ceilf(v.z), ceilf(v.w));
}
template <class T>
inline Vec4<T> abs(const Vec4<T>& v)
{
    return Vec4<T>(fabsf(v.x), fabsf(v.y), fabsf(v.z), fabsf(v.w));
}
template <class T>
...
Matrix4<T>& Matrix4<T>::perspective(T fovDegrees, T aspect, T n, T f)
{
    ...
    if (n == 0)
    {
        printf("Warning: creating a perspective matrix with near 0, this can result in depth test artifacts, use e.g. "
               "0.1\n");
    }

    printf("creating pers: fovy: %f (degrees), aspect: %f, near: %f, far: %f\n", fovDegrees, aspect, n, f);
...
[6]   Slerping Clock Cycles, J.M.P van Waveren,
http://software.intel.com/sites/default/files/m/d/4/1/d/8/293747_293747.pdf [7]   Slerp from GamePlay,
https://raw.github.com/blackberry/GamePlay/master/gameplay/src/Quaternion.cpp [8]   My previous implementation that also
supports mat3 and some other features: https://gist.github.com/roxlu/9fc5487a1c4e0794342b
...
inline Vec3<T> Quaternion<T>::transform(const Vec3<T>& v) const
{
    ...
    return Vec3<T>(p * v.x + m * x + c * (y * v.z - z * v.y),
...
struct Spline
{
    ...
    template <class I>
    void           assign(I begin, I end); /* assign multiple values; just like std::vector<T>::assign() */
...
#include <curl/curl.h>
#define RX_CHECK_CURLCODE(c, str)       \
    {                                   \
        if (c != CURLE_OK)              \
        {                               \
            printf("Error: %s\n", str); \
...
extern int  rx_load_jpg(std::string     filepath,
                        unsigned char** pix,
                        int&            width,
                        int&            height,
                        int&            nchannels,
                        int*            allocated = NULL);
extern bool rx_save_jpg(std::string    filepath,
                        unsigned char* pix,
                        int            width,
                        int            height,
                        int            nchannels,
                        ...
                        J_DCT_METHOD   dctMethod  = JDCT_FASTEST);
...
class Shader
{ /* represents a GL shader - works only with shaders loaded from file */
  ...
  public:
    ...
    ~Shader()
    {
    } /* @todo - we should clean the resources here */
    Shader& load(GLenum      type,
                 std::string filepath,
                 std::string extra = ""); /* load a shader file for the given type, you can pass extra data to the
                                             shader which is prepended */
...
class Program
{
  ...
  public:
    ...
    Program& create(GLenum      type,
                    std::string filepath,
                    std::string extra = ""); /* create a shader and add it, we take ownership you can pass extra data to
                                                the shader, which is prepended */
    Program& link(int          nattribs = 0,
                  const char** attribs  = NULL,
                  int          nfrags   = 0,
                  const char** frags =
                      NULL); /* compiles + links the shaders, if you want to bind attrib or frag locations pass them */
...
class Font
{
  ...
  public:
    ...
    bool  getChar(unsigned int code,
                  Character&   result); /* get a specific character, returns false when the font doesn't have the char */
...
static uint64_t freepixel_pixel_data[] = {0x0,
                                          0x0,
                                          0x0,
                                          0x0,
                                          0x0,
                                          ...
                                          0x0,
                                          0x0};
static int      freepixel_char_data[]  = {
    32,  121, 187, 3, 16, -1, 0, 7, 33,  126, 153, 1, 16, 3, 0, 7, 34,  26,  34,  5, 16, 1,  0, 7,
    35,  32,  34,  5, 16, 1,  0, 7, 36,  30,  0,   6, 16, 1, 0, 7, 37,  37,  0,   6, 16, 1,  0, 7,
    38,  38,  34,  5, 16, 1,  0, 7, 39,  126, 17,  1, 16, 3, 0, 7, 40,  125, 187, 2, 16, 3,  0, 7,
    41,  12,  204, 2, 16, 2,  0, 7, 42,  92,  170, 4, 16, 2, 0, 7, 43,  97,  170, 4, 16, 3,  0, 7,
    44,  15,  204, 2, 16, 2,  0, 7, 45,  80,  34,  5, 16, 1, 0, 7, 46,  126, 68,  1, 16, 3,  0, 7,
    47,  92,  34,  5, 16, 1,  0, 7, 48,  98,  34,  5, 16, 1, 0, 7, 49,  112, 170, 4, 16, 2,  0, 7,
    50,  110, 34,  5, 16, 1,  0, 7, 51,  116, 34,  5, 16, 1, 0, 7, 52,  122, 34,  5, 16, 1,  0, 7,
    53,  0,   51,  5, 16, 1,  0, 7, 54,  6,   51,  5, 16, 1, 0, 7, 55,  12,  51,  5, 16, 1,  0, 7,
    380, 108, 85,  5, 16, 1,  0, 7, 381, 114, 85,  5, 16, 1, 0, 7, 382, 120, 85,  5, 16, 1,  0, 7};
...
struct VertexPC
{
    ...
    VertexPC(vec3 pos, vec4 col)
        : pos(pos)
        , col(col)
    {
    }
    void set(vec3 p, vec4 c)
    {
        pos = p;
        col = c;
    }
...
{ /* Position Texcoord 3 */
    ...
    VertexPT3(vec3 p, vec3 t)
        : pos(p)
        , tex(t)
    {
    }
    void set(vec3 p, vec3 t)
    {
        pos = p;
        tex = t;
    }
...
struct VertexPTN
{
    ...
    VertexPTN(vec3 p, vec2 t, vec3 n)
        : pos(p)
        , tex(t)
        , norm(n)
    {
    }
    void set(vec3 p, vec2 t, vec3 n)
    {
        pos  = p;
        tex  = t;
        norm = n;
...
{
    VertexPTTN(){};
    VertexPTTN(vec3 p, vec2 tex, vec4 tan, vec3 norm)
        : pos(p)
        , tex(tex)
        , tan(tan)
        , norm(norm)
    ...
    void set(vec3 vp, vec2 vtex, vec4 vtan, vec3 vnorm)
    {
        pos  = vp;
        tex  = vtex;
        tan  = vtan;
...
class OBJ
{
  public:
    ...
    {
        int v, t, n, tan;
    }; /* v = vertex index, t = texcoord index, n = normal index, tan = tangent index */
    struct FACE
    {
        TRI a, b, c;
    };
...
class PainterContextPC
{
  ...
  public:
    PainterContextPC(Painter& painter); /* a PainterContext is used to render specific vertex data. This context renders
                                           only colors */
    ...
    void command(GLenum                 cmd,
                 std::vector<VertexPC>& vertices); /* adds a command (Painter calls this when using begin()/end() */
...
class PainterContextPT
{
  ...
  public:
    Painter&                    painter;  /* reference to the main Painter object */
    std::vector<PainterCommand> commands; /* the commands/texture draw calls. the type field contains the texture type;
                                             at this moment of writing only GL_TEXTURE_2D is supported */
...
class Painter
{
  ...
  public:
    ...
    void circle(float x,
                float y,
                float radius); /* draw a circle, see resolution() to change the resolution of the circle */
    void line(float x0,
              float y0,
              float x1,
              float y1); /* draw a single line, see begin()/end() if you want to draw line strips */
    void texture(GLuint tex,
                 float  x,
                 float  y,
                 float  w,
                 float  h); /* draw a texture, at the time of writing only GL_TEXTURE_2D is supported */
    ...
    void color(float r = 1.0,
               float g = 1.0,
               float b = 1.0,
               float a = 1.0); /* set the color of lines, circles, rectangles */
    void rgba(int r = 255,
              int g = 255,
              int b = 255,
              int a = 255); /* set the color of lines, circles, rectangles, etc.. using RGB values in range [0,255] */
    void hex(std::string hexstr); /* set the color of lines, circles, rectangles, etc.. using a HEX string, e.g.
                                     `484A47` for rgb, or 484A47FF for rgba. */
    void fill();                  /* enable fill mode */
    void nofill();                /* disable fill mode */
    void resize(int w,
                int h);     /* whenever your viewport changes, call this so we can recalculate the projection matrix */
  ...
  public:
    ...
    /* begin() - end() handling */
    GLenum command_type; /* when begin() is called we store the command type. the next call to vertex defines what
                            PainterContext we will use; for now only default is used */
    int context_type; /* when begin()/end() is used, a call to one of the overloaded vertex() function will define what
                         context will be used. in end() we add the added vertices to the correct context */
...
long audioplayer_data_cb(
    cubeb_stream* stm,
    void*         user,
    void*         buffer,
    long          nframes); /* The callback that gets called by cubeb that whenever it wants some more audio data */
void audioplayer_state_cb(cubeb_stream* stm,
                          void*         user,
                          cubeb_state   state); /* Is called whenever a stream starts or stops */

/* ----------------------------------- */

class AudioFile
{ /* An AudioFile will hold the raw loaded audio data */
  public:
    AudioFile();
...
{ /* Everytime you call AudioPlayer::play() we create a new AudioPlaybackState instance that keeps track of the read
     position for this sample */
  public:
    AudioPlaybackState(int name, AudioFile* file); /* C'tor; pass the name of the audio file and a pointer to the actual
                                                      AudioFile itself that contains the audio data */
    ~AudioPlaybackState();
...
class AudioPlayer
{ /* The AudioPlayer manages all the audio files and states. */
  public:
    AudioPlayer();                            /* C'tor, creates the necessary cubeb context and stream. */
    ~AudioPlayer();                           /* D'tor, cleans up all allocated members. */
    bool add(int name, std::string filepath); /* Add a new audio file for the given name and filepath. Note that we use
                                                 libsndfile to load the audio files. */
    AudioPlaybackState* play(int   name,
                             float volume = 1.0); /* Playback the given audio name. We return the state object on which
                                                     you can set the volume. On error we return NULL. */
  ...
  public:
    ...
    std::vector<AudioPlaybackState*>
        play_states; /* The playback states; each state keeps track of what frames to play and the volume */
...
#define RX_VERBOSE(fmt, ...)                                   \
    {                                                          \
        rx_verbose(__LINE__, __FUNCSIG__, fmt, ##__VA_ARGS__); \
    }
#define RX_WARNING(fmt, ...)                                   \
    {                                                          \
        rx_warning(__LINE__, __FUNCSIG__, fmt, ##__VA_ARGS__); \
    }
#define RX_ERROR(fmt, ...)                                   \
    {                                                        \
        rx_error(__LINE__, __FUNCSIG__, fmt, ##__VA_ARGS__); \
    }
#else
#define RX_VERBOSE(fmt, ...)                                           \
    {                                                                  \
        rx_verbose(__LINE__, __PRETTY_FUNCTION__, fmt, ##__VA_ARGS__); \
    }
#define RX_WARNING(fmt, ...)                                           \
    {                                                                  \
        rx_warning(__LINE__, __PRETTY_FUNCTION__, fmt, ##__VA_ARGS__); \
    }
#define RX_ERROR(fmt, ...)                                           \
    {                                                                \
        rx_error(__LINE__, __PRETTY_FUNCTION__, fmt, ##__VA_ARGS__); \
    }
#endif
...
int rx_load_jpg(std::string filepath, unsigned char** pix, int& width, int& height, int& nchannels, int* allocated)
{
    ...
    { /* When an error occurs we jump to this part. */
        if (NULL != fp)
        {
            fclose(fp);
            fp = NULL;
...
bool rx_save_jpg(std::string    filepath,
                 unsigned char* pix,
                 int            width,
                 int            height,
                 int            nchannels,
...
extern bool rx_save_png(std::string filepath, unsigned char* pixels, int w, int h, int channels = 3, bool flip = false)
{
...
Program& Program::create(GLenum type, std::string filepath, std::string extraSource)
{
    Shader* s = new Shader();
...
Program& Program::link(int nattribs, const char** atts, int nfrags, const char** fraglocs)
{
...
Shader& Shader::load(GLenum t, std::string fp, std::string extraSource)
{
...
extern GLuint rx_create_texture(std::string filepath, int internalFormat, int format, int type)
{
    ...
    , cube_stream(NULL)
{
    ...
    cubeb_stream_init(
        cube_ctx, &cube_stream, "stream", params, latency_ms, audioplayer_data_cb, audioplayer_state_cb, this);
}
